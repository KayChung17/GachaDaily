export function initGachaDaily() {
  if (window.__gachaDailyInitialized) return;
  window.__gachaDailyInitialized = true;
  const apiBase = (window.__GACHA_DAILY_API_BASE__ || "").replace(/\/$/, "");
  const defaultRssUrl = apiBase ? `${apiBase}/api/rss` : "/api/rss";
  const defaultSyncUrl = apiBase ? `${apiBase}/api/sync` : "/api/sync";
  ...
  function resolveRssFetchUrl(rawUrl) {
    ...
    // If user already points to our RSS proxy endpoint, keep it as-is.
    if (parsed.pathname.endsWith("/api/rss")) {
      return parsed.toString();
    }

    // Any cross-origin RSS source should be fetched through backend proxy.
  ...
  async function syncRssToPool() {
    ...
    try {
      ...
      if (error instanceof TypeError) {
        alert("RSS 同步失败：后端服务不可达，请确认 FastAPI 已启动在 http://127.0.0.1:8000");
        return;
      }
